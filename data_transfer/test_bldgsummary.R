...
c0coord=read.csv("/Users/ryanswart/Projects/PyProjects/DataTransfer/WRF_AutoBEM/sample_data_fromWRF_forAutoBEM/chi0_90m_wrfcoord2bldg.csv", header=TRUE)
...
for (i in c0$BLDGID)
  write.table(subset(chi0,coord %in% unlist(strsplit(subset(c0,BLDGID==i)$coord,","))) %>%
                ...
                summarise(T_K=mean(Temp.K.),
                          ...
                          WindDir_from=mean(WindDir.from.),
                          WindSpd_ms=mean(WindSpeed.ms.1.)),
              file=paste("/Users/ryanswart/Projects/PyProjects/DataTransfer/data_transfer/test_summaries/Chi0_BldgWeather_90m_",i,".csv",sep=""),row.names = FALSE, sep=",",quote=FALSE)
#
# for (i in c1$BLDGID)
